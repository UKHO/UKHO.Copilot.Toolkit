...
- For every future implementation report, require each canonical relationship field to have a matching direct one-hop local Markdown link. Validate the plan field/link pair as the expected accessible `implementation-plan` artifact in the exact same lifecycle folder; for remediation, validate the source Review-report, plan, and reviewed implementation-report field/link pairs as accessible expected-type artifacts in that same folder, with the reviewed report and source Review report resolving to the exact plan and reviewed-report chain. Fail closed on a missing, non-local, indirect, mismatched, wrong-type, inaccessible, or wrong-folder pair; do not discover, repair, or infer a predecessor.
