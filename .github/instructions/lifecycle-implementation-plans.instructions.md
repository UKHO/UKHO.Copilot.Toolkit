...
- For applicable Review workflows, record the exact associated Review-report path or contract: exactly one `<NNN>-review-report.md` per completed evidence-based pass, in the exact plan's same folder, independently allocated using matching-suffix two-scan/no-overwrite rules. Review reports are immutable, non-authorizing evidence; rejected inputs create no report.
...
- Include a dedicated `## Approved commands` section in every new or materially revised plan. Before finalizing the plan, consider each build, test, and script command required for planned implementation or validation. For every evidenced acceptable candidate, record its Tier, unchanged literal command, fixed workspace-contained working directory, purpose, expected output or result, known effects, inputs and preflight, fresh per-invocation developer approval point, post-command inspection, failure disposition, and rollback. Record an explicit unavailable/no-command state with its rationale when a required command is absent, unproven, unnecessary, or cannot satisfy the gates; do not use a category-only or inferred command as authorization.
- Keep Tier 1 and Tier 2 records separate. Tier 1 may cover only an evidenced local build, test, or script invocation. Tier 2 is a separately approved dependency-installation invocation with its own package, lockfile, configuration, source, and install-script preflight; it must not be bundled with Tier 1 or substituted for it. Approved-command records remain conditional authorization only: exact plan identity, Implement-only execution, fresh approval, and all applicable policy controls are still required.
