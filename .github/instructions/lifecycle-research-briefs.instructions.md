...
- Apply the shared [RPIR lifecycle core](../skills/rpir-lifecycle-core/SKILL.md) for canonical input, direct provenance, and phase-boundary semantics; this instruction retains Research-specific content, allocation, and write limits.
...
- Record the exact lifecycle folder and artifact path, in-progress/completed status, initial or revision metadata with amendment provenance, and, for each required source record, only its lifecycle-core-derived exact repository-relative canonical identity plus a direct validated one-hop renderable Markdown link to that same record. Do not persist a raw input alias, local absolute path, or Markdown destination as identity. The canonical path selects identity; the Markdown destination is evidence only, must render directly from the new record without traversal, URI, repair, redirect, or indirect route, and must never select a record. Allocation requires matching-suffix inventory inspection, `001` or maximum-valid-prefix-plus-one, immediate pre-create reinspection, and no overwrite; stop on malformed or inaccessible inventory or collision. Never infer the current or approved artifact from the highest prefix, recency, or suffix alone.
- Persist every new or amended Research brief only as `Status: In progress`; Research never sets `Completed`. Completed records are immutable and may be closed only prospectively through valid explicit `/plan` admission of that exact canonical in-progress brief. Do not migrate, reopen, or amend completed records.
...
- Preserve the selected exact numbered filename and lifecycle folder layout; use only lifecycle-core-derived exact repository-relative canonical identities for durable identity and handoffs, with a renderable local Markdown destination only as direct provenance evidence.
